...
//    @AutoUpdateTime(printLog = true)
    ...
    public void exportTokenAndRefreshTokenToCsv(int size, String fileName) {
        ...
        try (FileWriter writer = new FileWriter(filePath);
            ...
            // 写入CSV头
            pw.println("phone,token,refreshToken");
            // 写入数据
            for (int i = 0; i < PHONE_LIST.size(); i++) {
                String phone = PHONE_LIST.get(i);
                String token = TOKEN_LIST.get(i);
                String refreshToken = REFRESHTOKEN_LIST.get(i);

                pw.printf("%s,%s,%s%n", phone, token, refreshToken);
            }
...
//    }
//
//    private String generateUserToken(String phone,int expireTime,List<Long> userIds) throws Exception {
//        //先查用户是否存在
//
//        List<String> token  =new ArrayList<>();
//        for (int i = 0; i < userIds.size(); i++) {
//            token.add(jwtUtil.generateToken(userIds.get(i),1800L,ChronoUnit.SECONDS));
//        }
//        User user = new User();
//        user = query().eq("phone", phone).one();
//        //不存在就生成用户并且插入
