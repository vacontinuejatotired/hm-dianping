...
3. [层叠结构详解](#3-层叠结构详解)
   ...
   - [3.6 上下文层 —— UserHolder](#36-上下文层--userholder)
   - [3.7 前端层（参考）](#37-前端层参考)
4. [核心数据流](#4-核心数据流)
...
| **上下文安全** | 通过 `toolContext` 将当前用户 ID 注入工具调用 | 🔄 基础实现 |
...
| 工具方法 | 描述 | 参数 | 安全约束 |
|---------|------|------|---------|
| `queryPublishedBlogs` | 查询当前用户点赞前 10 篇博客 | `ToolContext` → 提取 `userId` | 仅查当前用户 |
| `publishTestBlog` | 发布一篇测试博客 | `ToolContext` → 提取 `userId` | 仅当前用户 |
